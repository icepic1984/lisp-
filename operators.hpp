...
template <typename T> struct tag_dispatch {
	typedef sexpr::basic_type type;
};

template <> struct tag_dispatch<sexpr::nil_type> {
	typedef sexpr::nil_type type;
};

template <> struct tag_dispatch<sexpr::invalid_type> {
	typedef sexpr::invalid_type type;
};

template <> struct tag_dispatch<sexpr::func_t> {
	typedef sexpr::function_type type;
};

template <> struct tag_dispatch<bool> {
	typedef sexpr::bool_type type;
};

template <> struct tag_dispatch<std::vector<sexpr>> {
	typedef sexpr::list_type type;
};

template <> struct tag_dispatch<lisp_string> {
	typedef sexpr::string_type type;
};

template <> struct tag_dispatch<std::string> {
...
struct sexpr_is_equal {
	...
	bool operator()(const A& a, const B& b) const
	{
		typename std::conditional<std::is_integral<A>::value &&
		                          ...
		                          std::true_type, std::false_type>::type select;
	...
	bool dispatch(const lisp_string& a, lisp_string& b, std::false_type) const
	{
		return a == b;
	}
...
template <typename Base> struct sexpr_arithmetic {
	...
	template <typename A> sexpr dispatch(const A& a, std::true_type) const
	{
		return Base::eval(a);
	}

	template <typename A> sexpr dispatch(const A& a, std::false_type) const
	{
		throw std::invalid_argument("Wrong type argument!");
		(void)a;
		return sexpr{};
	...
	template <typename A> sexpr operator()(const A& a) const
	{
		typename std::conditional<std::is_arithmetic<A>::value, std::true_type,
...
struct add_binary {
	template <typename A, typename B> static sexpr eval(const A& a, const B& b)
	{
		return sexpr(a + b);
	}
};
...
struct add_unary {
	template <typename A> static sexpr eval(const A& a)
	{
		return sexpr(+a);
	}
};
...
struct car_helper {
	...
	template <typename A> result_t operator()(const A&)
	{
...
struct nth_helper {
	...
	template <typename A> result_t operator()(const A&, std::size_t)
	{
