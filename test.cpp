...
BOOST_AUTO_TEST_CASE(cons_test)
{
	...
	expr =
	    evals(parse(tokenize("(cons (quote(a))  (quote(b c d))) ")), env.get());
...
BOOST_AUTO_TEST_CASE(begin_test)
{
	auto env = std::make_shared<environment>();
	auto expr =
	    evals(parse(tokenize("(begin (quote a) (quote b))")), env.get());
	std::stringstream buffer;
	buffer << expr;
...
BOOST_AUTO_TEST_CASE(set_test)
{
	...
	auto expr = evals(parse(tokenize("(define x 100)")), env.get());
	BOOST_CHECK_EQUAL(expr.get<int>(), 100);
	expr = evals(parse(tokenize("(define foo (lambda () (define x 120) x))")),
	             env.get());
	expr = evals(parse(tokenize("(foo)")), env.get());
	BOOST_CHECK_EQUAL(expr.get<int>(), 120);
	expr = evals(parse(tokenize("(define bar (lambda () (set x 4) x))")),
	             env.get());
                                       ...
                                       (lambda (amt)  \
                                         (begin \
                                           (set balance (+ balance amt))\
                                             balance))))")),
	             env.get());
	expr = evals(parse(tokenize("(define account (make-account 100))")),
	             env.get());
